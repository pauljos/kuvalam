...
// specific facts (e.g. "25 villas", "50 metres") that must be preserved exactly.
...
- For code skills, write clean, error-handled code that validates input.`
  ...
  fastify.get('/tenants/:tenantId/agents/:agentId/preview-prompt', {
    ...
    try {
      ...
      // Compose EXACTLY as the runtime task loop does (archetype template +
      // stripped user guardrails + injected goal) so the preview matches what's
      // actually sent to the LLM — including the task goal when provided.
      const { buildSystemPrompt } = await import('../services/task-reports.js')
      let fullPrompt = await buildSystemPrompt(agent, [], goal)
