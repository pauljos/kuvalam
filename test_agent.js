...
async function run() {
  try {
    const task = await dispatchTask({
      ...
      goal: 'Analyze recent cycle time data, create a chart, and push it to the dashboard integration at http://localhost:3001/api/reports',
      userId: 'system'
    });
    ...
    while(status === 'RUNNING' || status === 'QUEUED') {
      ...
      const { rows } = await query('SELECT status, result, error FROM agent_tasks WHERE id = $1', [task.id || task.taskId]);
      if (rows.length > 0) {
