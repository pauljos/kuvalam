...
async function run() {
  ...
  console.log("4. Clicking 'Push to Ollama' for flux-schnell-custom...");
  await page.evaluate(() => {
    const buttons = Array.from(document.querySelectorAll('button'));
    const pushBtn = buttons.find(b => b.textContent.includes('Push to Ollama') && b.closest('div')?.textContent?.includes('flux-schnell-custom'));
    if (pushBtn) pushBtn.click();
  });

  await new Promise(r => setTimeout(r, 8000));
  console.log("5. Taking screenshot after pushing FLUX.1 Schnell to Ollama...");
  await page.screenshot({ path: 'flux_pushed_to_ollama.png' });

  await browser.close();
  console.log("FLUX.1 Schnell Push to Ollama test complete!");
}
